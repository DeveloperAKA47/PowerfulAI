...
while True:
    ...
    if opt==1:
        ...
        if len(student_list)==0:
            ...
            pass_count=len([stu for stu in student_list if stu["分数"]>=60])
            fail_count=total_students-pass_count
            pass_rate=pass_count/total_students*100
            print(f"学生总人数:{total_students},成绩总分:{total_score},平均分:{average_score:.2f},最高分:{max_score},最低分:{min_score},及格人数:{pass_count},不及格人数:{fail_count},及格率:{pass_rate:.2f}%")
    elif opt==7:
        # 独立数据清空入口，复用删除模块核心逻辑，配备二次确认机制，操作不可逆，清空后实时反馈操作结果。
        confirm=input("确认清空所有学生数据？输入Y确认，N取消：")
        if confirm=="Y":
            student_list.clear()
            print("已清空所有学生数据")
        elif confirm=="N":
